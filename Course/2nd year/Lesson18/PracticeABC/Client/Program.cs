...
{ 
    ...
    //PracticeC
    const string url2 = "http://localhost:5087";
    const string DisplayProductsMethod = "/store/show";
    const string SendProductMethod = "/store/add";
    const string AuthMethod = "store/auth";


    static void DisplayProducts()
        {
            //var url = "http://localhost:5087/store/show"; // На всякий оставлю
            var client = new HttpClient();   
            var response = client.GetAsync(url2 + DisplayProductsMethod).Result;  
    ...
    {       
            ...
            var response = client.PostAsync(url2 + SendProductMethod, content).Result;
    ...
    {       //var url = "http://localhost:5087/store/auth"; // Замените на порт вашего сервера
            ...
            var response = client.PostAsync(url2 + AuthMethod, content).Result;
            if (response.IsSuccessStatusCode)
            {
                var responseContent = response.Content.ReadAsStringAsync().Result;
                Console.WriteLine(responseContent);
